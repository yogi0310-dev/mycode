...
def commandpush(devicecmd): # devicecmd==dict
    ...
    for ip in devicereboot.keys(): #looping through the dict
        print(f'{crayons.green("Connecting to")}..{ip}') # fstring with RED
        for myreboot in devicereboot[ip]:
            print(f'REBOOTING NOW --> {myreboot}')
    return None
...
def main():
    ...
    devicereboot = {"10.1.0.1":["reload"], "10.2.0.1":["reload"]}
    print (f'Welcome to the Network Device Command pusher') # Welcome Message

    ## Run
    commandpush(devicecmd) # call function to push command
